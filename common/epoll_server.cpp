...
EpollServer::EpollServer(int port, const Config& config) 
    : port_(port), listen_fd_(-1), epoll_fd_(-1), config_(config), running_(false) {
    
    std::cout << "[EpollServer] Creating server on port " << port_ 
              << " with " << config_.thread_pool_size << " worker threads" << std::endl;
    std::cout << "[EpollServer] Max events: " << config_.max_events << std::endl;
    std::cout << "[EpollServer] Max connections: " << config_.max_connections << std::endl;
    std::cout << "[EpollServer] Buffer size: " << config_.buffer_size << " bytes" << std::endl;
...
void EpollServer::run() {
    ...
    while (running_) {
        int nfds = epoll_wait(epoll_fd_, events.data(), config_.max_events, config_.epoll_timeout);
        
        if (nfds == -1) {
            if (errno == EINTR) continue; // 被信号中断
            std::cerr << "[EpollServer] epoll_wait failed: " << strerror(errno) << std::endl;
            break;
        }
...
bool EpollServer::sendResponse(int client_fd, const std::string& data) {
    ...
    ssize_t sent = send(client_fd, message.data(), message.length(), MSG_NOSIGNAL);
    if (sent > 0) {
        bytes_sent_ += sent;
...
bool EpollServer::setupListenSocket() {
    ...
    if (listen_fd_ < 0) {
        std::cerr << "[EpollServer] Failed to create socket: " << strerror(errno) << std::endl;
        return false;
    }
    ...
    if (setsockopt(listen_fd_, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        std::cerr << "[EpollServer] Failed to set SO_REUSEADDR: " << strerror(errno) << std::endl;
        return false;
    }
    
    // 设置非阻塞
    if (!setNonBlocking(listen_fd_)) {
        std::cerr << "[EpollServer] Failed to set listen socket non-blocking" << std::endl;
        return false;
    }
    ...
    if (bind(listen_fd_, (sockaddr*)&addr, sizeof(addr)) < 0) {
        std::cerr << "[EpollServer] Failed to bind: " << strerror(errno) << std::endl;
        return false;
    }
    
    // 开始监听
    if (listen(listen_fd_, 1024) < 0) {
        std::cerr << "[EpollServer] Failed to listen: " << strerror(errno) << std::endl;
        return false;
    }
...
bool EpollServer::setupEpoll() {
    ...
    if (epoll_fd_ < 0) {
        std::cerr << "[EpollServer] Failed to create epoll: " << strerror(errno) << std::endl;
        return false;
    }
    ...
    if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, listen_fd_, &ev) < 0) {
        std::cerr << "[EpollServer] Failed to add listen fd to epoll: " << strerror(errno) << std::endl;
        return false;
    }
...
void EpollServer::handleNewConnection() {
    while (true) {
        ...
        if (client_fd < 0) {
            ...
            std::cerr << "[EpollServer] Accept failed: " << strerror(errno) << std::endl;
            break;
        }
        
        // 检查连接数限制
        if (active_connections_.load() >= config_.max_connections) {
            std::cout << "[EpollServer] Connection limit reached, rejecting new connection" << std::endl;
            close(client_fd);
            continue;
        }
        ...
        if (!setNonBlocking(client_fd)) {
            std::cerr << "[EpollServer] Failed to set client socket non-blocking" << std::endl;
            close(client_fd);
            continue;
        }
        ...
        if (epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, client_fd, &ev) < 0) {
            std::cerr << "[EpollServer] Failed to add client fd to epoll: " << strerror(errno) << std::endl;
            close(client_fd);
            continue;
        }
        ...
        std::cout << "[EpollServer] New connection from " << client_ip 
                  ...
                  << " (fd=" << client_fd << ", active=" << active_connections_.load() << ")" << std::endl;
    }
}
...
void EpollServer::handleClientData(int client_fd) {
            ...
            if (connection_handler_) {
                ...
                thread_pool_->enqueue([this, client_fd, message]() {
                    try {
                        ...
                        std::cerr << "[EpollServer] Handler error: " << e.what() << std::endl;
                        ++failed_requests_;
                    }
                });
            }
    ...
    if (bytes_read == 0) {
        ...
        // 读取错误
        std::cerr << "[EpollServer] Read error on fd " << client_fd << ": " << strerror(errno) << std::endl;
        closeConnection(client_fd);
    }
}
...
std::vector<std::string> EpollServer::extractMessages(std::shared_ptr<ClientConnection> conn, 
    ...
    while (conn->receive_buffer.length() >= sizeof(uint32_t)) {
        ...
        // 提取消息
        std::string message = conn->receive_buffer.substr(sizeof(uint32_t), message_len);
