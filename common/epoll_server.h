...
class EpollServer {
public:
    using ConnectionHandler = std::function<void(int fd, const std::string& data)>;
...
private:
    struct ClientConnection {
        ...
        explicit ClientConnection(int f) : fd(f), last_activity(std::chrono::steady_clock::now()) {}
    };
    ...
    // 消息解析（处理TCP粘包）
    std::vector<std::string> extractMessages(std::shared_ptr<ClientConnection> conn, 
